...
float test::BRKGA_(string dir, string fileName)
{
	Solution s;
	perm_rep a;
	a.loadInstance(dir, fileName, 3);

	const unsigned n = a.inst->numC;		// size of chromosomes
	...
	Decoder decoder(a);			// initialize the decoder
	...
	double f = algorithm.getBestFitness();
