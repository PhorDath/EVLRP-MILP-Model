	...
	for (auto i : V0) {
		for (auto j : V1) {
			if (i.key != j.key) {
				...
				fo += dist(i, j) * model.getVarByName(var);
			}
		}
	}
	...
	for (auto i : C0) {
		for (auto j : V1) {
			if (i.key != j.key) {
				...
				GRBLinExpr c = ti + (getTD(i, j) + i.serviceTime) * xij;
	...
	for (auto i : C0) {
		for (auto j : V1) {
			if (i.key != j.key) {
				...
				GRBLinExpr c = ti + (getTD(i, j) + i.serviceTime) * xij; // verify getS
...
{
	return inst->nodes.at(key).serviceTime;
}
